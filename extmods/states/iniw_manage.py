...
__virtualname__ = "ini_without_spaces"
...
def __virtual__():
    ...
    """
    return __virtualname__ if "ini_without_spaces.set_option" in __salt__ else False
...
def options_present(name, sections=None, separator="=", strict=False):
    ...
    try:
        ...
        if sections:
            ...
            cur_ini = __salt__["ini_without_spaces.get_ini"](name, separator)
            ...
            if __opts__["test"]:
                ...
                options_updated = __salt__["ini_without_spaces.set_option"](name, options, separator)
            ...
            if strict:
                for opt_to_remove in set(original_top_level_opts).difference(options):
                    if __opts__["test"]:
                        ...
                        __salt__["ini_without_spaces.remove_option"](
            ...
            for section_name, section_body in [
                ...
                if __opts__["test"]:
                    ...
                    options_updated = __salt__["ini_without_spaces.set_option"](
            ...
            if not __opts__["test"]:
                changes = __salt__["ini_without_spaces.set_option"](name, sections, separator)
...
def options_absent(name, sections=None, separator="="):
    ...
    if __opts__["test"]:
        ...
        for section in sections or {}:
            ...
            try:
                cur_section = __salt__["ini_without_spaces.get_section"](name, section, separator)
            ...
            if isinstance(sections[section], list):
                ...
                option = section
                if not __salt__["ini_without_spaces.get_option"](name, None, option, separator):
                    ret["comment"] += "Key {} does not exist.\n".format(option)
                    continue
                ret["comment"] += "Deleted key {}.\n".format(option)
                ret["result"] = None
    ...
    for section, keys in sections.items():
        for key in keys:
            try:
                current_value = __salt__["ini_without_spaces.remove_option"](
...
def sections_present(name, sections=None, separator="="):
    ...
    try:
        changes = __salt__["ini_without_spaces.set_option"](name, section_to_update, separator)
    except OSError as err:
        ret["result"] = False
        ret["comment"] = "{}".format(err)
        return ret
...
def sections_absent(name, sections=None, separator="="):
    ...
    for section in sections or []:
        try:
            cur_section = __salt__["ini_without_spaces.remove_section"](name, section, separator)
        except OSError as err:
            ret["result"] = False
            ret["comment"] = "{}".format(err)
            return ret
