...
class CustomDumper(yaml.SafeDumper):
    ...
    def represent_dict_preserve_order(self, data):
        return self.represent_dict(
            sorted([x for x in data.items()], key=lambda z: str(z))
        )
...
CustomDumper.add_representer(
    collections.OrderedDict, CustomDumper.represent_dict_preserve_order
)
...
def generated(name, vm_manifest, only=None):
    ...
    for vm, data in sorted(vm_manifest.items()):
        ...
        del data["dom0"]
        key = (
            "qvm.present" if data.get("vm_managed_by_formation", True) else "qvm.exists"
        )
        vmid = "VM " + vm
    ...
    if code == 0:
        ...
        for statekey, statereturn in data.items():
            for k in ["duration", "__run_num__", "__sls__", "start_time"]:
                del statereturn[k]
            try:
                if statereturn.get("retcode", 0) != 0 or statereturn["result"] not in [
                    True,
                    None,
                ]:
                    ret["result"] = False
